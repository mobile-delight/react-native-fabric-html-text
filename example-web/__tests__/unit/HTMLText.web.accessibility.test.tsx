...
/**
 * Accessibility tests for HTMLText.web screen reader navigation.
 *
 ...
 */
describe('HTMLText.web Accessibility', () => {
  ...
  describe('Link semantic HTML', () => {
    it('should render links as real <a> elements with href', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://example.com">Example Link</a></p>' />
      );
    ...
    it('should preserve link text as accessible name', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://example.com">Click Here</a></p>' />
      );
    ...
    it('should maintain multiple links as distinct elements', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://a.com">First</a>, <a href="https://b.com">Second</a>, and <a href="https://c.com">Third</a></p>' />
      );
  ...
  describe('Link position information', () => {
    it('should add aria-describedby for link position (link 1 of 3)', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://a.com">First</a> <a href="https://b.com">Second</a> <a href="https://c.com">Third</a></p>' />
      );
  ...
  describe('Focus management', () => {
    ...
    it('links should be natively focusable', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://example.com">Focusable Link</a></p>' />
      );
  ...
  describe('Link type accessibility', () => {
    it('should preserve mailto: links for email actions', () => {
      const { container } = render(
        <HTMLText html='<p>Email: <a href="mailto:test@example.com">test@example.com</a></p>' />
      );
    ...
    it('should preserve tel: links for phone actions', () => {
      const { container } = render(
        <HTMLText html='<p>Call: <a href="tel:+1234567890">+1 234 567 890</a></p>' />
      );
  ...
  describe('Empty state accessibility', () => {
    it('should not have link-related ARIA when no links', () => {
      const { container } = render(<HTMLText html="<p>No links here.</p>" />);
  ...
  describe('Nested link content accessibility', () => {
    it('should preserve nested formatting inside links for screen readers', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://example.com"><strong>Bold</strong> Link</a></p>' />
      );
    ...
    it('should maintain link semantics with nested elements', () => {
      const { container } = render(
        <HTMLText html='<p><a href="https://example.com"><em>Emphasized</em> text</a></p>' />
      );
