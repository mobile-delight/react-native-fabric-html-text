...
function writeAndroidStrings(androidLocale, strings) {
  // Use 'values' for default locale (empty string), 'values-XX' for specific locales
  const dirName = androidLocale ? `values-${androidLocale}` : 'values';
  const dirPath = path.join(rootDir, `android/src/main/res/${dirName}`);
