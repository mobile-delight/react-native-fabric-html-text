...
export default function HTMLText({
  ...
  // Re-sanitize after adding aria-describedby attributes (defense-in-depth)
  // Note: aria-describedby is in ALLOWED_ATTRIBUTES so it's preserved
  const sanitizedContent = sanitize(processedHtml);

  // Append trusted description elements AFTER sanitization
  // These are generated by our code (not user input) and require inline styles
  // for visually-hidden screen reader text
  const finalHtml = sanitizedContent + descriptionElements;
  ...
  return (
    <div
      ...
      role={role}
      // nosemgrep: no-dangerous-innerhtml-without-sanitization - sanitizedContent is sanitized via sanitize(); descriptionElements are trusted generated code
